...
### 决策 9：报告结构 = 元信息区 + 多平台分组 + 用例折叠表格

（2026-09-10 用户评审后升级，替换早期「单用例按步骤平铺」结构）

- **上半部分（元信息）**：标题 + 报告 ID；关键指标一行（总用例 / 成功 / 失败 / 通过率）；元信息一行（运行时间 / 模型 / 总 token / 浏览器）。其余信息（报告名、平台、耗时、环境）一律不展示。
- **下半部分（用例区）**：按**平台分组**——每个平台一个独立块，块内是该平台的用例列表；每个用例一行，标注成功/失败，**点击展开**对应步骤 → LLM 子步骤 → 状态/截图，可收起（accordion 折叠）。
- **多平台预留**：当前 runner 一次跑一个用例（= 单平台单用例）；数据模型设计为「平台块 → 用例列表」，未来批量跑多平台时天然可聚合，无需改渲染层。
- 数据模型改为：`Report`（meta + platforms）→ `PlatformBlock`（name + cases）→ `CaseBlock`（name/status/duration + steps）→ `ReportStep`/`ReportSubStep`（沿用对齐/判定/截图）。

### 决策 10：视觉方向 = Editorial（编辑科技）

用户要求「年轻化、互联网化」的报告视觉，按 huashu-design 三方向门出了三版真实初稿，最终用户选定 **C · Editorial（编辑科技）**（`direction-approved.md` 记录原话）。三版对比：

- **A · Fresh（清新活力）**：大圆角 + 柔和渐变卡片 + 圆环通过率——轻快但偏「通用 SaaS」，辨识度不足。
- **B · Glass（未来玻璃）**：渐变底 + 玻璃拟态卡片——科技感强，但玻璃拟态在长报告中视觉噪音偏大。
- **C · Editorial（编辑科技）★ 选定**：衬线大标题（Georgia/宋体）+ 细线分隔 + 无卡片大数字横排 + 电光蓝 accent，克制、有杂志感、信息密度高且耐读——最贴合「工程报告」的正式气质。

C 方向视觉规范（`render.py` 落地，字体用系统栈保证离线可用）：

- 背景 `#fbfaf7`（暖白）/ 卡片 `#fffdfa` / 文字 `#141414` / 次要 `#8a857a` / 细线 `#e6e1d6`。
- accent `#0a5cff`（电光蓝）；成功 `#0f8a4d` / 失败 `#d22730` / 未执行 `#a8a093`。
- 标题与关键数字用衬线（`Georgia, "Songti SC", serif`），正文用 sans（`-apple-system, "PingFang SC", sans-serif`）。
- 指标区无卡片、大数字横排 + 细竖线分隔；用例列表用上下细线分隔（账本感）；截图直角深色占位。

## Open Questions

- 暂无阻塞项。后续可考虑：报告是否需要内嵌「失败步骤的历史错误信息全文」、是否支持历史报告对比（diff）、批量多平台运行时的报告聚合接口。
